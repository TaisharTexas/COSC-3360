...
namespace opMode{
    ...
    void executeActions(map<string, int> &letterVars, map<string, int> p_Vars, vector<vector<string> > actionMap){
        ...
        // cout << "Letter Vars Key-Value Pairs:" << endl;
        ...
        // }
        // cout << "P-Vars Key-Value Pairs:" << endl;
                    ...
                    case 2:
                        ...
                        for(int r = 0; r < actionMap.size(); r++){
                            ...
                            if(actionMap[r][1] == "p2"){
                                ...
                                if(actionMap[r][0].size() > 1){
                                    //LHS is a p-var, wait for correct pid to show complete
                                    cout << "case 2 -- value = " << (actionMap[r][0]) << endl;
                                    cout << "case 2 -- logic#1 = " << (actionMap[r][0] == "p0") << endl;
                                    cout << "case 2 -- logic#2 = " << (actionMap[r][0] == "p1") << endl;
                                    if(actionMap[r][0] == "p0"){
                                        cout << "case 2 - p0 RHS fired" << endl;
                                        int status;
                                        // waitpid(pid[0], &status, 0);
                                        wait(NULL);
                                        close(fd[0][1]);
                                        ...
                                        read(fd[1][0], &LHS, sizeof(int));
                                        close(fd[1][0]);
                                        cout << "case 2 - p1 RHS fired. -- LHS = " << LHS << endl;
                                    }
                                    else if(actionMap[r][0] == "p3"){
                                        int status;
                                        // waitpid(pid[3], &status, 0);
    ...
    if(varVals.length() > 1 && pipelineVals.length() > 0){
        ...
        for(int i = 0; i< pipelineLines_str.size(); i++){
            ...
            if(strstr(tempLine, "input_var") != NULL){ //we found the line of input vars
                ...
                string RHS;
                string operation;
                if(targetIndex == 4){//input var to internal var
                ...
                else if(targetIndex == 5){//internal var to internal var
